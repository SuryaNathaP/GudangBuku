...
<div class="page-card">
    <div class="page-card-header">
        <div class="page-card-title">Data Kategori</div>
        <div class="header-actions">
            <input type="text" id="searchInput" class="search-input" placeholder="Cari kategori...">
            <a href="{{ route('kategoris.add') }}" class="add-btn">
    ...
    <div style="overflow-x: auto;">
        <table class="tbl">
            ...
            <tbody>
                ...
                <tr>
                    <td><span class="id-badge">{{ $kategori->firstItem() + $loop->index }}</span></td>
                    <td style="font-weight:500;">{{ $k->nama_kategori }}</td>
                    <td style="color: var(--text-secondary);">{{ Str::limit($k->keterangan, 60) }}</td>
                    <td>
                        <div class="row-actions">
                            <a href="{{ route('kategoris.edit', $k->id) }}" class="act-edit">Edit</a>
                            <form action="{{ route('kategoris.delete', $k->id) }}" method="POST" style="display:inline;">
                                @csrf
                                <button type="button" class="act-del" style="background:none; border:none; padding:0; cursor:pointer; font-family:inherit;"
                                    onclick="confirmDelete(this.closest('form'), '{{ addslashes($k->nama_kategori) }}')">Hapus</button>
                            </form>
                        </div>
                    </td>
                </tr>
                @empty
                <tr class="empty-row"><td colspan="4">Tidak ada data kategori.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
